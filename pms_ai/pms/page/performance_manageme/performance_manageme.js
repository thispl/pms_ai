frappe.pages['performance-manageme'].on_page_load = function(wrapper) {
    ...
    $(page.body).html(frappe.render_template("performance_manageme", {}));
    let $page = $(page.body);
    ...
    let unit_field = frappe.ui.form.make_control({ df: { label: "Department", fieldtype: 'Link', options: 'Unit', fieldname: 'department', change: () => trigger_dashboard_reload() }, parent: $page.find('#filter-area'), render_input: true });
    ...
    function load_dashboard() {
        ...
        if (unit_field.get_value()) {
            filters.push(["department", "=", unit_field.get_value()]);
        }
        ...
        frappe.call({
            ...
            args: {
                ...
                fields: [
                    ...
                    "department", "custom_grade", "docstatus", "start_date",
    ...
    frappe.set_route("List", "Appraisal", filters);
                ...
                Object.keys(tableData).forEach(dept => {
                    ...
                    deptNames.push(dept.replace(" - GALFAR", "").trim());
                    completedPerc.push(comp);
                    pendingPerc.push(pend);
                    tableRows += `<tr style="transition:background 0.3s;"
                        onmouseover="this.style.background='#f0f8ff'"
                        onmouseout="this.style.background='white'">
                        <td style="padding:10px;text-align:left;">${dept.replace(" - GALFAR", "").trim()}</td>
                        <td>${d.total}</td>
                        <td>${comp}%</td>
                        <td>${pend}%</td>
                    </tr>`;
