frappe.pages['pms-dashboard'].on_page_load = function(wrapper) {
    ...
    function hideChrome() {
    // Remove 'expanded' to collapse sidebar to icon-only strip
    $('.body-sidebar-container').removeClass('expanded');
    // Prevent the toggle button from re-expanding it on this page
    $('.sidebar-toggle-btn').off('click.pms').on('click.pms', function(e) {
...
function restoreChrome() {
    // Re-enable normal toggle behavior when leaving
    $('.sidebar-toggle-btn').off('click.pms');
}

hideChrome();

// Keep collapsed if Frappe re-adds 'expanded'
const sidebarObserver = new MutationObserver(function() {
    if (frappe.get_route()[0] === 'pms-dashboard') {
        $('.body-sidebar-container').removeClass('expanded');
    }
});
sidebarObserver.observe(
    document.querySelector('.body-sidebar-container') || document.body,
    { attributes: true, attributeFilter: ['class'], subtree: false }
);

// Restore + disconnect when leaving
$(wrapper).on('hide', function() {
    ...
    /* HIDE STRIP THE SIDEBAR CONTEXT FROM DESK OUTRIGHT */
.layout-side-section,
.desk-sidebar,
.page-sidebar,
.sidebar-toggle-btn,
.close-sidebar-btn,
.layout-side-section.widget-area {
    display: none !important;
}

/* Expand main content to fill the full width since sidebar is gone */
